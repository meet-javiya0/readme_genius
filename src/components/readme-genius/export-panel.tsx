...
interface ExportPanelProps {
  ...
  fileName?: string;
  onMarkdownDownloaded?: () => void;
}

export function ExportPanel({ getMarkdownContent, fileName = 'README', onMarkdownDownloaded }: ExportPanelProps) {
  ...
  const handleExportMarkdown = () => {
    ...
    onMarkdownDownloaded?.(); // Notify that markdown has been "saved" by downloading
  };
  ...
  const handleExportHtml = () => {
    ...
    // Links and Images
    htmlContent = htmlContent.replace(/!\[(.*?)\]\((.*?)\)/gim, '<img src="$2" alt="$1" style="max-width:100%;" data-ai-hint="illustration diagram" />');
    ...
    htmlContent = htmlContent.replace(/^(\s*\d+\.\s+.+\n?)+/gim, (match) => {
